...
func _perform_card_swap_animation(played_card: Node, opponent_card: Node) -> void:
	...
	var hand_slots_node = get_node_or_null("/root/main/Parallax/HandSlots")
	if not hand_slots_node or not hand_slots_node is Node2D:
		push_error("GameManager: HandSlots node not found at /root/main/Parallax/HandSlots - required for swap animation")
		return
