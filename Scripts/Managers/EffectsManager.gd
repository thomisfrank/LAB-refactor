...
func _execute_peek_hand(card_node: Node) -> void:
	...
	if game_manager.has_method("enter_selection_mode"):
		...
		print("[EffectsManager] enter_selection_mode called for PEEK_HAND. opponent_count=", opponent_cards.size(), " played_card=", card_node.get_path())
...
func _on_peek_deck_keep(temp_peek_card: Node, played_card: Node):
	...
	if is_instance_valid(played_card):
		...
		# (original flow: the played card was disintegrated/queued above)
