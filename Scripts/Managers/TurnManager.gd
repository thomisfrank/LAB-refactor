...
@export var action_pop_padding: float = 4.0
...
func _pop_action_icon_for_side(side_name: String, index: int) -> void:
	...
	# Do pop tween (quick grow then settle) - pulse scale only (no rotation)
	if "rect_scale" in target:
		# For Control nodes, also nudge the icon away from its neighbor while scaled to avoid overlap
		var original_pos = target.rect_position if "rect_position" in target else Vector2.ZERO
		var icon_width = target.rect_size.x if "rect_size" in target else 32
		var shift = (action_pop_scale - 1.0) * icon_width * 0.5 + action_pop_padding
		var shift_dir = -1 if (index % 2) == 0 else 1
		var tween = create_tween()
		# Grow scale and move outward in parallel
		...
		tween.parallel().tween_property(target, "rect_position", original_pos + Vector2(shift_dir * shift, 0), action_pop_grow_time).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
		# Settle back to original scale and position
		...
		tween.parallel().tween_property(target, "rect_position", original_pos, action_pop_settle_time).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_IN)
	elif "scale" in target:
		# For Node2D targets, nudge using position
		var original_pos2 = target.position if "position" in target else Vector2.ZERO
		var icon_width2 = 32
		var shift2 = (action_pop_scale - 1.0) * icon_width2 * 0.5 + action_pop_padding
		var shift_dir2 = -1 if (index % 2) == 0 else 1
		...
		tween2.parallel().tween_property(target, "position", original_pos2 + Vector2(shift_dir2 * shift2, 0), action_pop_grow_time).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
		...
		tween2.parallel().tween_property(target, "position", original_pos2, action_pop_settle_time).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_IN)
